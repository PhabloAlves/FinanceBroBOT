...
from app.services.google_sheets import save_sheet, set_renda, clear_data, get_resume
...
async def telegram_webhook(request: Request):
    ...
    if text.startswith("/despesa") or text.startswith("/receita"):
        ...
        if len(partes) < 2:
            ...
            return {"ok": True}
        await set_renda(partes[1])
        await send_message(f"✅ Renda mensal definida: R$ {partes[1]}")

    elif text == "/resumo":
        await send_message(await get_resume())
